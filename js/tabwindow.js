function TabWindow(tab, url) {
  ...
  tab.tabWindow = $("<div>").load("browser.html", function() {
    ...
    $(webview).ready(function() {
      ...
      if (url == "webexpress://newtab") {
        webview.loadURL(`file://${__dirname}/page.html`);
      } else {
        webview.loadURL(url);
      }
    ...
    history.click(function(e){
      ...
      Ripple.makeRipple($(this), relX , relY, $(this).width(), $(this).height(), rippleTime, 0);
      var tab = new Tab();
      var tw = new TabWindow(tab, `file://${__dirname}/history.html`);
      addTab(tw, tab);
    });
    ...
    searchInput.keypress(function (e) {
      if (e.which == 13) {
        ...
        if (!searchInput.val().startsWith("webexpress://")) {
          ...
          if (searchInput.val().startsWith("webexpress://history")) {
            webview.loadURL(`file://${__dirname}/history.html`);
          }
          if (searchInput.val().startsWith("webexpress://settings")) {
            webview.loadURL("settings.html");
          }
          if (searchInput.val().startsWith("webexpress://extensions")) {
            webview.loadURL("extensions.html");
          }
          if (searchInput.val().startsWith("webexpress://bookmarks")) {
            webview.loadURL("bookmarks.html");
          }
          if (searchInput.val().startsWith("webexpress://downloads")) {
            webview.loadURL("downloads.html");
          }
        }
